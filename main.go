...
func filter1(openBarPrevious, closeBarPrevious, highBarCurrent, highBarPrevious, lowBarCurrent, lowBarPrevious float64) bool {
	if math.Abs(openBarPrevious-closeBarPrevious) < 50 {
...
func strategy(klines []models.Kline) (string, float64) {
	if len(klines) > 1 {
		...
		if (openBarCurrent < openBarPrevious) && (closeBarCurrent > openBarPrevious) {
			...
			// 过滤器1
			res1 := filter1(openBarPrevious, closeBarPrevious, highBarCurrent, highBarPrevious, lowBarCurrent, lowBarPrevious)
			if !res1 {
				return "过滤器1不做多单", 0
			}
			// 过滤器2
		...
		if (openBarCurrent > openBarPrevious) && (closeBarCurrent < openBarPrevious) {
			...
			// 过滤器1
			res := filter1(openBarPrevious, closeBarPrevious, highBarCurrent, highBarPrevious, lowBarCurrent, lowBarPrevious)
			if !res {
				return "过滤器1不做空单", 0
			}
			// 过滤器2
...
func main() {
	//
	tk1 := toolbox.NewTask("tk1", "0 0 */1 * * ?", f1HourUsdt)
	tk2 := toolbox.NewTask("tk2", "0 0 */4 * * ?", f4HourUsdt)
	tk3 := toolbox.NewTask("tk3", "0 0 */1 * * ?", f1HourUsd)
	tk4 := toolbox.NewTask("tk4", "0 0 */4 * * ?", f4HourUsd)
